import { Button, Image, Text, TextInput, useWindowDimensions, View } from 'react-native';
...
export const MenuOption: React.FC<MenuOption> = ({ item, updateMenuLists, deleteMenuLists }) => {
    ...
    const updateOptionCate = (element: OptionListProps) => {
        setOption((prevOptions) =>
            prevOptions.map((opt) => (opt.optionListPk === element.optionListPk ? element : opt))
        );
    };
    ...
    return (
        <View style={styles.menuItem}>
            ...
            <View style={{ flexDirection: 'row', gap: 5 }}>
                ...
                <Button title="삭제" onPress={() => deleteMenuLists(item.menu_pk)} />
            </View>
                ...
                <View style={{ flexDirection: 'row', gap: 5, alignItems: 'center' }}>
                    <View style={{ margin: 10 }}>
                        <TextInput
                            ...
                            placeholder="옵션 카테고리"
                            onChangeText={(newText) => setForm({ ...form, opSubject: newText })}
            ...
            {isUpdate && (
                <View style={{ backgroundColor: '#D9D9D9', padding: 5, gap: 5 }}>
                    <TextInput
                        style={{ height: 40, backgroundColor: 'white' }}
                        placeholder="메뉴 이름"
                        onChangeText={(newText) => setMenuItem({ ...menuItem, subject: newText })}
                        defaultValue={menuItem.subject}
                    />
                    <TextInput
                        style={{ height: 40, backgroundColor: 'white' }}
                        placeholder="메뉴 설명"
                        onChangeText={(newText) => setMenuItem({ ...menuItem, description: newText })}
                        defaultValue={menuItem.description}
                    />
                    <TextInput
                        style={{ height: 40, backgroundColor: 'white' }}
                        placeholder="메뉴 이미지"
                        onChangeText={(newText) => setMenuItem({ ...menuItem, imageURL: newText })}
                    ...
                    <View style={{ width: '100%', justifyContent: 'space-between', flexDirection: 'row' }}>
                        <View style={{ flexDirection: 'row', gap: 5 }}>
                            <Button
                                title="주류 여부"
                                onPress={() => setMenuItem({ ...menuItem, isAlcohol: !menuItem.isAlcohol })}
                            />
                            {!menuItem.isAlcohol ? (
                            ...
                            <Button
                                title="대표 메뉴 여부"
                                onPress={() => setMenuItem({ ...menuItem, isBestMenu: !menuItem.isBestMenu })}
                            />
